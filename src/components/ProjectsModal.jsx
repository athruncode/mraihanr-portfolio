                ...
                >
                  {project.images.length === 1 ? (
                    <img
                      ...
                      alt={project.name}
                      className="w-[90%] md:w-[75%] max-h-full object-contain drop-shadow-[0_20px_40px_rgba(0,0,0,0.6)] cursor-pointer hover:scale-105 transition-transform duration-300 pointer-events-auto"
                      onClick={() => setLightboxData({ images: project.images, index: 0 })}
                    />
                  ) : (
                    <div className="relative w-full h-full flex items-center justify-center group/mockups pointer-events-auto">
                      <img
                        src={project.images[0]}
                        alt={`${project.name} 1`}
                        className="absolute w-[60%] md:w-[60%] object-contain -rotate-[12deg] md:-rotate-[8deg] -translate-x-16 md:-translate-x-20 -translate-y-8 md:translate-y-0 drop-shadow-[0_15px_30px_rgba(0,0,0,0.5)] z-0 scale-95 cursor-pointer group-hover/mockups:-translate-x-20 md:group-hover/mockups:-translate-x-40 group-hover/mockups:-rotate-[15deg] group-hover/mockups:scale-100 hover:!scale-110 hover:!z-20 transition-all duration-500 ease-out"
                        onClick={() => setLightboxData({ images: project.images, index: 0 })}
                      />
                      <img
                        src={project.images[1]}
                        alt={`${project.name} 2`}
                        className="absolute w-[65%] md:w-[65%] object-contain rotate-[8deg] md:rotate-[4deg] translate-x-12 md:translate-x-12 translate-y-12 md:translate-y-12 drop-shadow-[0_25px_50px_rgba(0,0,0,0.8)] z-10 cursor-pointer group-hover/mockups:translate-x-16 md:group-hover/mockups:translate-x-32 group-hover/mockups:translate-y-16 group-hover/mockups:rotate-[10deg] group-hover/mockups:scale-100 hover:!scale-110 hover:!z-20 transition-all duration-500 ease-out"
                        onClick={() => setLightboxData({ images: project.images, index: 1 })}
                      />
                    </div>
                  )}
                </motion.div>
            ...
            >
              <AnimatePresence mode="wait">
                <motion.img
                  ...
                  alt={`Enlarged mockup ${lightboxData.index + 1}`}
                  className="w-full md:w-auto max-h-[85vh] md:max-h-[90vh] object-contain rounded-lg shadow-[0_0_50px_rgba(0,0,0,0.8)]"
              ...
              {lightboxData.images.length > 1 && (
                <button
                  ...
                  className="absolute left-4 md:left-10 top-1/2 -translate-y-1/2 w-10 h-10 md:w-14 md:h-14 bg-black/50 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-colors border border-white/20 backdrop-blur-md z-[110]"
                >
              ...
              {lightboxData.images.length > 1 && (
                <button
                  ...
                  className="absolute right-4 md:right-10 top-1/2 -translate-y-1/2 w-10 h-10 md:w-14 md:h-14 bg-black/50 hover:bg-white/20 rounded-full flex items-center justify-center text-white transition-colors border border-white/20 backdrop-blur-md z-[110]"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5 md:w-8 md:h-8">
              ...
              <button
                onClick={() => setLightboxData(null)}
                className="absolute top-6 right-6 md:top-10 md:right-10 w-10 h-10 bg-white/20 hover:bg-white/40 rounded-full flex items-center justify-center text-white transition-colors border border-white/20 backdrop-blur-md z-[110]"
              >
